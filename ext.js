...
function dragHandler(event, path){
	var csInterface = new CSInterface();
	...
	*/
	...
	if (extPath !== null){
		extPath = path;
		// extPath = extPath + '/payloads/cmlogo.png';
		// Z:/ART/Assets/3. Still/2. Overlay/2. Logos/2. Watermarks
...
function outroDragHandler(event, link){
	// alert(link);
	var csInterface = new CSInterface();
	var extPath 	= link;
	var OSVersion	= csInterface.getOSInformation();
...
function generatePath(){
	options = document.getElementById('outroDimensions').options;
	...
	path = rootFolder+kind;
	buildPath(path);
		function buildPath(path, outro){
			// alert("building path: "+path);
			generateDropdownLink("outroDimensions", path);
			outro = document.getElementById('outroDimensions').value;
			while (outro == "") {
				items = readDir(path);
				for (var i = 0; i < items.length; i++) {
					if (items[i].includes("enders")){
						items[0] = items[i];
					}
					else if (items[i].includes("1. English")) {
						items[0] = document.getElementById("language").value;
					}
				}
				if (items[0] == undefined) break;
				path = path+"/"+items[0];
				buildPath(path);
			}
		}
/*
		items = readdir(path);
		if (generateDropdownLink("outroDimensions", path) == undefined) path = path+lang;
	...
	getSubfolder(path);
		function getSubfolder(path){
			...
			// alert(cont.join(", "))
			cont = cont[0];
			path = path+cont+"/3. Renders";
			generateLink(path);						
		}*/
}

function generateDropdown(id, parent, array, doSomething){
	...
	for (var i = 0; i < options.length; i++) {
		...
		if (doSomething) option.label = doSomething(option.label);
		select.add(option);
...
function generateFileDropdown(id, parent, array, doSomething){
	var select = document.createElement("select");
	select.setAttribute("id", id);
	var options = array;
...
function generateDropdownLink(id, path){
	var array = readDir(path);
	var select = document.getElementById(id);
	var formats = ["scape", "ortrait", "quare", "2-3"];
	for (var i = 0; i < formats.length; i++) {
		filtered = array.filter(file => file.includes(formats[i]));
		if (filtered[0] != undefined) {select.options[i].value = path+"/"+filtered[0]};
	}
}

function readDir(path){
	var folder = window.cep.fs.readdir(path);
		var entries = folder.data;
		var fileNames = [];
		var folderNames = [];
		return folder.data
...
function remvoeExtension(string){
	var string = string;
	string = string.substr(0, -4);
	return string
}

function folderNameCleanup(string){
	var string = string;
